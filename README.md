...
微软拼音里常见的可导入 DAT 主要有两类：用户自定义短语 DAT 和自学习词汇 DAT。它们都能让输入法识别自定义词条，但入口、编码方式和适用场景不同。

用户自定义短语 DAT 用于“用户自定义短语”功能，适合导入姓名、固定短语、常用机构名等明确词条。本工具生成这类 DAT 时默认使用拼音首字母编码，例如 `张三 -> zs`。

自学习词汇 DAT 用于微软拼音的“自学习词汇”导入导出，适合导入输入法学习词库形式的词条。本工具生成这类 DAT 时默认使用全拼编码，例如 `张三 -> zhang san`。这类 DAT 不适合使用首字母编码。

简单判断：

- 想导入到 Windows 设置里的“用户自定义短语”，选择“用户自定义短语 DAT”
- 想处理微软拼音自学习词汇导入导出，选择“自学习词汇 DAT”
